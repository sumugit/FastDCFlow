...
if __name__ == '__main__':
    ...
    for t in ts:
        ...
        for query_instance in tqdm(query_features):
            ...
            x_cf, y_cf = cf_vae.compute_elbo(query_instance, expected_outcome, predictive_model, t)
            x_cf = torch.hstack((x_cf, y_cf))
            x_cf_df = pd.DataFrame(x_cf.detach().cpu().numpy(), columns=feature_names + [target])
            x_cf_df.drop(target, axis=1).to_csv(configuration_for_proj["cfs_t_cf_vae_targetenc_" + DATA_NAME] +  f"t_{int(t)}_neg_{negative_cnt}.csv", index=False)

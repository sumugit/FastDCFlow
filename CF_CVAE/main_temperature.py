...
if __name__ == '__main__':
    ...
    negative_index = negative_prediction_index(labels, 1.0) # pred < 0.5 idx
    ...
    for t in ts:
        ...
        for query_instance, query_encoded_label in tqdm(zip(query_features, query_encoded_labels), total=len(query_features)):
            ...
            x_cf, y_cf = cf_cvae.compute_elbo(query_instance, expected_outcome, query_encoded_labels, predictive_model, t)
            x_cf = torch.hstack((x_cf, y_cf))
            x_cf_df = pd.DataFrame(x_cf.detach().cpu().numpy(), columns=feature_names + [target])
            x_cf_df.drop(target, axis=1).to_csv(configuration_for_proj["cfs_t_cf_cvae_targetenc_" + DATA_NAME] +  f"t_{int(t)}_neg_{negative_cnt}.csv", index=False)

...
class CF_CVAE(nn.Module):
    def __init__(self, encoder, decoder):
        super(CF_CVAE, self).__init__()
    ...
    def compute_loss(self, x, expected_outcome, label, predictive_model, temperature=1.0):
        # resize c for compute encoder and decoder
        expected_outcome = expected_outcome.view(expected_outcome.shape[0], 1)
        # encoder
        mean, logvar = self.encoder(x, expected_outcome, label)
        ...
        # decoder
        z = self.sample_latent_code(mean, logvar, temperature)
        x_cf = self.decoder(z, expected_outcome, label) # (batch_size, data_size)
        # Proximity
        recon_err = self.loss_prox(x, x_cf)
        # yloss
        y_cf = predictive_model(x_cf)

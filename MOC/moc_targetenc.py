...
from moc import MOC
# MOCO
from moc_problem import MyProblem
# pymoo library
...
if __name__ == '__main__':
    ...
    for query_instance in tqdm(query_features):
        ...
        start2 = time.time()
        # model
        exp_genetic = MOC(encoder_normalize_data_catalog, predictive_model, query_instance, TOTAL_CFS)
        ...
        x_cf = random.sample(list(x_cf), args.total_cfs)
        x_cf_df = pd.DataFrame(x_cf, columns=feature_names + [target])
        end2 = time.time()
        x_cf_df.drop(target, axis=1).to_csv(configuration_for_proj["cfs_raw_moc_targetenc_" + DATA_NAME] + f"neg_{negative_cnt}.csv", index=False)
        ...
        x_cf_df.to_csv(configuration_for_proj["cfs_moc_targetenc_" + DATA_NAME] + f"neg_{negative_cnt}.csv", index=False)
        loss_df.to_csv(configuration_for_proj["cfs_moc_targetenc_loss_" + DATA_NAME] + f"neg_{negative_cnt}.csv", index=False)
        # pareto set and front
        # ps = problem.pareto_set(use_cache=False, flatten=False)
        # pf = problem.pareto_front(use_cache=False, flatten=False)
        history['yloss'].append(problem.history['yloss'])
        history['proximity_loss'].append(problem.history['proximity_loss'])
        history['total_loss'].append(problem.history['total_loss'])
    ...
    with open(configuration_for_proj['moc_targetenc_' + DATA_NAME + '_history'], 'w') as f:
        json.dump(history, f, indent=2)
    with open(configuration_for_proj['moc_targetenc_' + DATA_NAME + '_times'], 'w') as f:
        json.dump(times, f)

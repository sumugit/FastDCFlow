...
if __name__ == '__main__':
    ...
    if PRETRAINED:
        flow_model = torch.load(configuration_for_proj['fastdcflow_model_targetenc_uniform_' + DATA_NAME])
        ...
        for epoch in tqdm(range(1, EPOCHS+1)):
            ...
            for local_batch, local_labels in (negative_loader):
                ...
                # add purturbation
                delta = torch.rand(z_value.shape, device=args.device)
        ...
        save_pytorch_model_to_model_path(flow_model, configuration_for_proj['fastdcflow_model_targetenc_uniform_' + DATA_NAME])
    ...
    for query_instance in tqdm(query_features):
        ...
        # add purturbation
        delta = torch.rand(z_value.shape, device=args.device) * args.temperature
        perturbed_z_value = make_perturbation(z_value, delta)
        x_cf = original_space_value_from_latent_representation(flow_model, perturbed_z_value)
        ...
        x_cf_df.drop(target, axis=1).to_csv(configuration_for_proj["cfs_raw_fastdcflow_targetenc_uniform_" + DATA_NAME] + f"neg_{negative_cnt}.csv", index=False)        
        ...
        x_cf_df.to_csv(configuration_for_proj["cfs_fastdcflow_targetenc_uniform_" + DATA_NAME] + f"neg_{negative_cnt}.csv", index=False)
        end3 = time.time()
